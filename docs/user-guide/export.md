...
1. 选择一个或多个联系人/群聊，或选择“全部导出”后按需仅保留全部群聊或全部联系人。
2. 选择时间范围和消息类型。“全部导出”同样支持今天、最近 3 天、最近 7 天、自定义时间和全部时间。
3. 选择格式；只有 HTML 可以配置媒体资源、语音转写和 ZIP。
4. 按需要设置头像、原图/缩略图和缺失资源处理。
5. 设置文件名并开始导出。
6. 在导出任务中心查看读取、解析、媒体处理、转写、写入和压缩进度；完成后打开文件位置。

“全部导出”会按所选时间范围和消息类型筛选聊天，只为存在匹配消息的群聊或联系人生成档案，不会生成空聊天目录。再次使用相同名称执行全部导出时，会按本次筛选结果刷新会话目录；普通单聊或多聊 HTML 导出仍采用增量合并。
